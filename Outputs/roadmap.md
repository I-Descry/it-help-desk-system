...
- [ ] **Phase 2 — Automation.** Power Automate flow(s) covering:
  ...
  - [x] `AssignmentMode` branching (manual Teams card vs. auto round-robin) — built and tested, all sub-paths.
  - [x] Writing `Assigned To` / `Status` back to the list — built and tested, both branches.
  - [ ] `ActingApprover` field + Manual-card delegation logic
